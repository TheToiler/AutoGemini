...
use crate::models::agents::agent_traits::{ FactSheet, ProjectScope, SpecialFunctions, ExecuteFunction };
...
impl AgentSolutionArchitect {
    ...
    async fn retrieve_project_scope(&mut self, factsheet: &mut FactSheet) -> ProjectScope {
        ...
        let ai_response: ProjectScope = ai_task_request_decoded::<ProjectScope>(msg_context, &self.attributes.position, get_function_string!(print_project_scope), print_project_scope).await;
    ...
    async fn retrieve_determine_external_urls(&mut self, factsheet: &mut FactSheet, msg_context: String) {
        let ai_response: Vec<String> = ai_task_request_decoded::<Vec<String>>(msg_context, &self.attributes.position, get_function_string!(print_site_urls), print_site_urls).await;
...
impl ExecuteFunction for AgentSolutionArchitect {
            ...
            match self.attributes.state {
                AgentState::Discovery => {
                    ...
                    if project_scope.is_external_urls_required {
                        self.retrieve_determine_external_urls(factsheet, factsheet.project_description.clone()).await;
                        self.attributes.state = AgentState::UnitTesting;
                    }
                },
                AgentState::UnitTesting => {
                    ...
                    for url in urls {
                        ...
                        PrintCommand::UnitTest.print_agent_message(self.attributes.position.as_ref(), endpoint_str.as_str());
                    ...
                    if !exclude_urls.is_empty() {
                        let new_urls: Vec<String> = factsheet.external_urls
                                                        .as_ref()
                                                        .unwrap()
                                                        .iter()
...
mod tests {
    ...
    async fn tests_solutions_architect() {
        ...
        agent.execute(&mut fact_sheet).await.expect("Failed to execute on the agent");
